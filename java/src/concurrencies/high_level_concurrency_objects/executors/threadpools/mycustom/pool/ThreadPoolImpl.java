...
import threadpools.mycustom.pool.common4j.DefaultThreadFactory;
import threadpools.mycustom.pool.common4j.ILifeCycle;
import threadpools.mycustom.pool.common4j.StringUtil;
import threadpools.mycustom.pool.job.ThreadPoolStateJob;
import threadpools.mycustom.pool.job.ThreadStackJob;
import threadpools.mycustom.pool.job.ThreadStateJob;
...
public class ThreadPoolImpl implements ILifeCycle, ThreadPool {
	...
	public void init() {
		if (ThreadPoolStatus.UNINITIALIZED != _status) {
			_logger.warn("initialization thread pool failed, because the status was wrong, "
					+ "current status was {} (0:UNINITIALIZED, 1:INITIALITION_SUCCESSFUL, "
					+ "2:INITIALITION_FAILED, 3:DESTROYED)", _status);
			return;
		}
