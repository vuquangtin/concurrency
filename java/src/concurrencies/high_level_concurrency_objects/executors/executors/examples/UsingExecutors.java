...
public class UsingExecutors {
	...
	public static void usingSingleThreadExecutor() {
		...
		singleThreadExecutor.execute(() -> System.out
				.println("and this one to."));
	...
	public static void usingCachedThreadPool() {
		...
		for (int i = 0; i < 10; i++) {
			Future<UUID> submittedUUID = cachedThreadPool.submit(() -> {
				...
				System.out.println("UUID " + randomUUID + " from "
		...
		cachedThreadPool.execute(() -> uuids.forEach((f) -> {
			try {
				System.out.println("Result " + f.get() + " from thread "
						+ Thread.currentThread().getName());
	...
	public static void usingFixedThreadPool() {
		...
		fixedPool.execute(() -> uuids.forEach((f) -> {
			try {
				System.out.println("Result " + f.get() + " from "
						+ Thread.currentThread().getName());
			} catch (InterruptedException | ExecutionException e) {
				e.printStackTrace();
			}
	...
	public static void usingScheduledThreadPool() {
		...
		scheduledThreadPool.scheduleAtFixedRate(
				() -> System.out.println("1) Print every 2s"), 0, 2,
				TimeUnit.SECONDS);
		scheduledThreadPool.scheduleAtFixedRate(
				() -> System.out.println("2) Print every 2s"), 0, 2,
				TimeUnit.SECONDS);
		scheduledThreadPool.scheduleWithFixedDelay(
				() -> System.out.println("3) Print every 2s delay"), 0, 2,
				TimeUnit.SECONDS);
	...
	public static void usingSingleTreadScheduledExecutor() {
		...
		singleThreadScheduler.scheduleWithFixedDelay(
				() -> System.out.println("2) Print every 2s delay"), 0, 2,
				TimeUnit.SECONDS);
	...
	public static void usingWorkStealingThreadPool() {
		...
		try {
			List<Future<UUID>> futureUUIDs = workStealingPool
					.invokeAll(severalUUIDsTasks);
