 ...
 *
 */
public class MultiPoolExecutor implements ExecutorService {
	...
	void init() {
		...
		for (int i = 0; i < n; i++) {
			...
			BlockingQueue<Runnable> bq = new ArrayBlockingQueue<Runnable>(numThreads);
	...
	// Executes the given tasks, returning a list of Futures holding their
	// status and results when all
	// complete or the timeout expires, whichever happens first.
	public <T> List<Future<T>> invokeAll(java.util.Collection<? extends java.util.concurrent.Callable<T>> tasks,
			long timeout, java.util.concurrent.TimeUnit unit) throws InterruptedException {
		return getBest().invokeAll(tasks, timeout, unit);
	}

	// Executes the given tasks, returning the result of one that
	// has completed successfully (i.e., without throwing an exception), if any
	// do.
	...
	// successfully
	// (i.e., without throwing an exception), if any do before the given timeout
	// elapses.
