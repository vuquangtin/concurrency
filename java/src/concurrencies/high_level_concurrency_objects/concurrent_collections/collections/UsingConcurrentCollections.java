...
public class UsingConcurrentCollections {
	...
	public static void usingConcurrentHashMap() {
		...
		for (int i = 0; i < 100; i++) {
			if (i % 6 == 0) {
				...
				// read
				executor.execute(() -> System.out.println("Printed "
						+ valuesPerUuid.values().toString()));
			}
		}
	 ...
	 * 
	 * Creates a new copy every time that the list is modified, only
	 * synchronizing briefly to ensure array content visibility.
	 * 
		...
		Runnable runConsumer = () -> {
			while (!Thread.currentThread().isInterrupted()) {
				try {
					...
					System.out.println("Consumed: " + uuid + " by "
		...
		Runnable runProducer = () -> {
			try {
				while (!Thread.currentThread().isInterrupted()) {
					...
					System.out.println("Produced: " + randomUUID + " by "
							+ Thread.currentThread().getName());
