...
public class UsingIntrinsicLocks {
	...
	public synchronized void mySynchronizedMethod() {
		state = !state;
		// Everything in this method can only be accessed by the thread who hold
		// the
		// lock.
		System.out.println("My state is:" + state);
		// Without sync: states have no order guarantee true, true, false,
		// true...
	...
	public void mySynchronizedBlock() {
		/*
		 * Everything in this block can only be accessed by the thread who hold
		 * the lock. The message bellow will be printed before the message
		 * inside the synchronized block
		 */
		System.out.println("Who owns my lock: "
				+ Thread.currentThread().getName());
		synchronized (this) {
			state = !state;
			System.out.println("Who owns my lock after state changes: "
					+ Thread.currentThread().getName());
