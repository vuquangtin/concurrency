...
public final class ExecutorServiceUtils {
	...
	public static ExecutorServiceWrapper newFixedThreadPool(String poolName) {
		...
		if (!StringUtils.isEmpty(poolName)) {
			Integer maxPoolSizeObject = maxProcessorsPerPool.get(poolName
					.toLowerCase());
		 ...
		 *
		 * By doing so, when the program exits the main() method - and
		 * regardless of whether System.exit() has been called - Java will not
		 * treat the active threads as blocking the termination.
		 *
		 * This means that the shutdown hook (which is added below) will be run,
		 * causing the graceful termination of the ExecutorService, and the
		 * running tasks.
		 *
		 * Without Daemon threads, the program will not terminate, nor will the
		 * shutdown hooks be called unless System.exit is called explicitly.
		 */
		...
		if (service != null) {
			ExecutorServiceWrapper wrapper = new ExecutorServiceWrapper(
					service, poolName);

			/**
			 * Shutdown hook to gracefully terminate the ExecutorService. Gives
			 * any existing tasks a chance to complete before forcing
			 * termination.
			 */
	...
	static void shutdown(ExecutorServiceWrapper wrapper) {
		...
		try {
			...
			while (!service.awaitTermination(
					params.getShutdownWaitCycleInSecs(), TimeUnit.SECONDS)) {
				long completedTasks = ExecutorServiceUtils
						.getCompletedTaskCount(service);
				if (completedTasks > -1 && completedTasks == lastCompletedTasks) {
					System.err
							.println("Waiting for shutdown of translation service. Completed "
									+ completedTasks
									+ " tasks out of "
					...
					if (stalledCount > params.getMaxStalledShutdownCycles()) {
						System.err.println("Waited "
								+ params.getShutdownStalledWaitTimeInSecs()
								+ " seconds without progress. Abandoning.");
						service.shutdownNow();
						if (!service.awaitTermination(
								params.getShutdownWaitCycleInSecs(),
								TimeUnit.SECONDS)) {
							break;
						}
					}
			...
			System.out.println("[" + wrapper.poolName
					+ "] Queue had max size of: " + wrapper.maxQueueCount);
		}
	}
	...
	public static class ExecutorServiceWrapper {
		...
		public <T> Future<T> submit(Callable<T> task) {
			try {
				...
				maxQueueCount = Math.max(maxQueueCount,
						ExecutorServiceUtils.getQueueSize(service));
			}
		}
	...
	public static class ExecutorShutdownParams {
		...
		private int maxStalledShutdownCycles = shutdownStalledWaitTimeInSecs
				/ shutdownWaitCycleInSecs;
		...
		public ExecutorShutdownParams(int shutdownStalledWaitTimeInSecs) {
			...
			maxStalledShutdownCycles = shutdownStalledWaitTimeInSecs
					/ shutdownWaitCycleInSecs;
		}

		public ExecutorShutdownParams(int shutdownStalledWaitTimeInSecs,
				int shutdownWaitCycleInSecs) {
			this.shutdownStalledWaitTimeInSecs = shutdownStalledWaitTimeInSecs;
