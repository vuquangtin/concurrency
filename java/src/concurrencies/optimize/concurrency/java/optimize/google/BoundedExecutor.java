 ...
 *
 * Differs from Executors#newFixedThreadPool by not holding on to idle threads
 * and by sourcing threads from another Executor rather than creating them
 * itself.
 *
 * Combined with Executors#newCachedThreadPool, the achieved semantics is that
 * of an unlimited task queue that scales the number of threads to a certain
 * limit, while also allowing those threads to time out. These semantics cannot
 * be achieved with ThreadPoolExecutor, as that class will always create new
 * threads up to the core size, even if allowing core threads to time out.
 * 
 * 
