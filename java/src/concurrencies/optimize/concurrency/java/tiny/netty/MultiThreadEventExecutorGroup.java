 ...
 *
 */
public abstract class MultiThreadEventExecutorGroup extends
	...
	protected MultiThreadEventExecutorGroup(int nThreads,
		...
		this.readonlyChildren = Arrays.asList(children);
		this.chooser = EventExecutorChooserFactory.INSTANCE
				.newChooser(children);
	}

	protected abstract EventExecutor newChild(EventExecutorGroup parent,
			ThreadFactory threadFactory);
	...
	public boolean isShuttingDown() {
		return readonlyChildren.stream()
				.allMatch(EventExecutor::isShuttingDown);
	}
	...
	public CompletableFuture<?> shutdownGracefully(long quietPeriod,
		...
		readonlyChildren.forEach(child -> child.shutdownGracefully(quietPeriod,
				timeout, timeUnit));
