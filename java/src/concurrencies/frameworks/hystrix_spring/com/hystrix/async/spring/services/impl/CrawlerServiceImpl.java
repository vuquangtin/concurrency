...
import com.hystrix.async.spring.dto.CrawledUrlsDTO;
...
public class CrawlerServiceImpl implements CrawlerService {
	...
	@HystrixCommand(commandKey = "CrawlerApiCommandKey", fallbackMethod = "handleFailureResponse_fallback", commandProperties = {
	...
	public CrawledUrlDetailsDTO getAlllinksUnderSameDomain(String url,
			int depth, List<String> navigatedUrls) {
		LOGGER.debug("fetching navigations");
		if (depth < 0) {
			return null;
		} else {
			List<String> updatedNavigatedUrls = Optional.ofNullable(
					navigatedUrls).orElse(new ArrayList<>());
			if (updatedNavigatedUrls.contains(url)) {
				return null;
			} else {
				updatedNavigatedUrls.add(url);
				final CrawledUrlDetailsDTO crawledUrlDetails = new CrawledUrlDetailsDTO(
						url);
				crawlForUrls(url)
						.ifPresent(
								crawledUrl -> {
																		...
																		navigation -> {
																			if (navigation
																				...
																				if (navigation
																						...
																						.equals(ANCHOR)) {
																					crawledUrlDetails
																							.addNodesItem(getAlllinksUnderSameDomain(
																									navigation
																											.attr("abs:href"),
																									depth - 1,
																									updatedNavigatedUrls));
																				} else if (navigation
																						.tag()
																						.getName()
																						.equals(IMAGES)
																						|| navigation
																								...
																								.getName()
																								.equals(SCRIPT)) {
																					CrawledUrlDetailsDTO crawledUrlDetailsDTO = new CrawledUrlDetailsDTO(
																							navigation
																									.attr("abs:src"));
																					crawledUrlDetails
																							.addNodesItem(crawledUrlDetailsDTO);
																				} else if (navigation
																						.tag()
																						.getName()
																						.equals(LINK)) {
	...
	public Optional<List<CrawledUrlsDTO>> crawlForUrls(final String url) {
		try {
			...
			LOGGER.error(
					String.format("Error getting contents of url %s", url), e);
			return Optional.empty();
		}
	}

	private CrawledUrlDetailsDTO handleFailureResponse_fallback(String url,
			int depth, List<String> navigatedUrls) {
		final CrawledUrlDetailsDTO crawledUrlDetails = new CrawledUrlDetailsDTO(
				url);
