	...
	public static void main(String[] args) {
		...
		Observable.create(subscriber -> {
			try {
				...
				.onErrorResumeNext(e -> Observable.just("Fallback Data")).subscribe(System.out::println);
		...
		Observable.create(subscriber -> {
			throw new RuntimeException("failed!");
		}).onErrorResumeNext(Observable.just("fallback value")).subscribe(System.out::println);
		...
		Observable.create(subscriber -> {
			...
			return attempts.zipWith(Observable.range(1, 3), (throwable, i) -> i).flatMap(i -> {
