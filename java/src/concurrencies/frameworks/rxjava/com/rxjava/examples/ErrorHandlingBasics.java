...
public class ErrorHandlingBasics {
	...
	public static void main(String... args) {
		 ...
		 */
		Observable.error(new RuntimeException("failed")).onErrorResumeNext(Observable.just("5) data"))
				.subscribe(System.out::println, t -> System.out.println("5) Error: " + t));

		/*
		 * Or the Throwable can be obtained to do conditional logic
		 */
		Observable.error(new IllegalStateException("failed")).onErrorResumeNext(t -> {
		...
		Observable.error(new RuntimeException("failed")).onErrorResumeNext(t -> {
			if (t instanceof IllegalStateException) {
				return Observable.error(t);
			} else {
