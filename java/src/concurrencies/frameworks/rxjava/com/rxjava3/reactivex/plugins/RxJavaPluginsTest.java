...
package com.rxjava3.reactivex.plugins;
...
public class RxJavaPluginsTest {
	...
	public void assemblyTrackingObservable() {
		...
		//
		// AssemblyStackTraceException aste =
		// AssemblyStackTraceException.find(ex);
		//
		// assertNotNull(aste);
		//
		// assertTrue(aste.getMessage(),
		// aste.getMessage().contains("createObservable"));
		//
	...
	public void assemblyTrackingSingle() {
		...
		//
		// assertTrue(aste.getMessage(),
		// aste.getMessage().contains("createSingle"));
		//
		// RxJavaPlugins.clearAssemblyTracking();
		//
		// ts = TestSubscriber.create();
	...
	public void assemblyTrackingCompletable() {
		...
		//
		// assertTrue(aste.getMessage(),
		// aste.getMessage().contains("createCompletable"));
		//
		// RxJavaPlugins.clearAssemblyTracking();
		//
		// ts = TestSubscriber.create();
	...
	public void overrideInitSingleScheduler() {
		final Scheduler s = Schedulers.single(); // make sure the Schedulers is
													// initialized
	...
	public void overrideInitComputationScheduler() {
		final Scheduler s = Schedulers.computation(); // make sure the
														// Schedulers is
														// initialized
		Supplier<Scheduler> c = new Supplier<Scheduler>() {
			@Override
	...
	public void overrideInitIoScheduler() {
		final Scheduler s = Schedulers.io(); // make sure the Schedulers is
												// initialized;
		Supplier<Scheduler> c = new Supplier<Scheduler>() {
			@Override
			public Scheduler get() throws Exception {
	...
	public void overrideInitNewThreadScheduler() {
		final Scheduler s = Schedulers.newThread(); // make sure the Schedulers
													// is initialized;
		Supplier<Scheduler> c = new Supplier<Scheduler>() {
			@Override
			public Scheduler get() throws Exception {
	...
	public void completableStart() {
		try {
			RxJavaPlugins
					.setOnCompletableSubscribe(new BiFunction<Completable, CompletableObserver, CompletableObserver>() {
						@Override
	...
	public void onErrorWithSuper() throws Exception {
		try {
			...
			BiFunction<? super Completable, ? super CompletableObserver, ? extends CompletableObserver> completableObserver2completableObserver = new BiFunction<Completable, CompletableObserver, CompletableObserver>() {
				@Override
				public CompletableObserver apply(Completable completable, CompletableObserver completableObserver)
						throws Exception {
	...
	public void clearIsPassthrough() {
		try {
			...
			// RxJavaPlugins.onObservableReturn(subscription));
			//
			// assertNull(RxJavaPlugins.onSingleReturn(null));
			//
			// assertSame(subscription,
			// RxJavaPlugins.onSingleReturn(subscription));
			//
			...
			//
			// Completable.CompletableOperator cop = new
	...
	public void onXError() {
		...
		//
		// Function<Throwable, Throwable> errorHandler = new Function<Throwable,
		// Throwable>() {
		// @Override
		...
		//
		// assertSame(ex, RxJavaPlugins.onObservableError(new
		// TestException("Forced failure 1")));
		//
		// assertSame(ex, RxJavaPlugins.onSingleError(new TestException("Forced
		// failure 2")));
		//
		// assertSame(ex, RxJavaPlugins.onCompletableError(new
		// TestException("Forced failure 3")));
		//
	...
	public void onPluginsXError() {
		...
		// final TestException ex = new TestException();
		//
		// final Function<Throwable, Throwable> errorHandler = new
		...
		// });
		//
		// RxJavaPlugins.getInstance().registerSingleExecutionHook(new
		...
		// });
		//
		// RxJavaPlugins.getInstance().registerCompletableExecutionHook(new
	...
	public void onXLift() {
		...
		// });
		//
		// RxJavaPlugins.setOnCompletableLift(new Function<CompletableOperator,
		// CompletableOperator>() {
		// @Override
	...
	public void onPluginsXLift() {
		...
		// final int[] counter = { 0 };
		//
		// final Function<Operator, Operator> onObservableLift = new
		...
		// };
		//
		// final Function<CompletableOperator, CompletableOperator>
		// onCompletableLift = new Function<CompletableOperator,
		// CompletableOperator>() {
		// @Override
		// public CompletableOperator call(CompletableOperator t) {
		// counter[0]++;
		...
		// RxJavaObservableExecutionHook() {
		// @Override
		// public <T, R> Operator<? extends R, ? super T> onLift(Operator<?
		// extends R, ? super T> lift) {
	...
	public void overrideConnectableObservable() {
		try {
			RxJavaPlugins
					.setOnConnectableObservableAssembly(new Function<ConnectableObservable, ConnectableObservable>() {
						@Override
		...
		try {
			...
			try {
				...
				if (!(ex.getCause() instanceof IllegalArgumentException)) {
					fail(ex.getCause().toString()
							+ ": Should have thrown NullPointerException(IllegalArgumentException)");
				}
			}
			...
			try {
				...
				if (!(ex.getCause() instanceof RuntimeException)) {
					fail(ex.getCause().toString()
							+ ": Should have thrown NullPointerException(RuntimeException(IOException))");
				}
	...
	public void isBug() {
		...
		// assertTrue(RxJavaPlugins.isBug(new UndeliverableException(new
		// TestException())));
		// assertTrue(RxJavaPlugins.isBug(new CompositeException(new
		// TestException())));
		// assertTrue(RxJavaPlugins.isBug(new OnErrorNotImplementedException(new
		// TestException())));
	}
}
