...
public class Intermediate implements Action {
	...
	public void run() throws Exception {
		...
		Observable.fromIterable(list).lift(new SwitchCaseBreak<>(caseBlocks)).subscribe(System.out::println);
		//Utils.sleep10000();
...
/**
 * An {@link ObservableOperator} that enables the switch-case behavior in
 * Rx-Java, it uses a table look up technique to simulate a switch-case behavior
 * <p>
 * you must supply a {@link Map} that holds functions that will be executed if
 * the emitted item matches the key they are mapped to
 * <p>
 * if multiple emitted items matches several keys, all there functions will be
 * executed and will emit there results, if you want the first match to disable
 * the rest of the checks, you can use {@link SwitchCaseBreak} instead
 * <p>
 * sample code :
 * <p>
 * {@code List<Integer> list = Arrays.asList(1, 2, 3, 4);}<br>
 * {@code Map<Integer, Function<Integer, String>> caseBlocks = new HashMap<>(2);}
 * <br>
 ...
 *
 * @param <T>
 *            the type of the items that will be passed to the {@code switch}
 *            statement
 * @param <R>
 *            the type of the items that will be returned from the {@code case}
 *            block execution
 */
class SwitchCase<T, R> implements ObservableOperator<R, T> {
	...
	/**
	 * create a {@link SwitchCase} operator, if any {@link Function} returned
	 * {@code null}, the whole operation will crash
	 *
	 * @param caseBlocks
	 *            the map that holds {@link Function} that are the case-blocks
	 *            in this switch-case
	 */
	...
	private Observer<T> createResultObserver(final Observer<? super R> observer) {
		return new Observer<T>() {
			...
			public void onNext(@NonNull T switchOn) {
				...
				Function<T, R> block = Maybe.just(caseBlocks).filter(map -> map.containsKey(switchOn))
						.map(map -> map.get(switchOn)).blockingGet();
...
/**
 * same as {@link SwitchCase} but it emits only one item which is the result of
 * the {@link Function} mapped to the key matching the first emitted value
 * <p>
 * sample code :
 * <p>
 * {@code List<Integer> list = Arrays.asList(1, 2, 3, 4);}<br>
