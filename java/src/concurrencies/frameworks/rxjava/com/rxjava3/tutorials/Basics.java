...
public class Basics implements Action {
	...
	private void printCollectionOfNumbers() {
		...
		Observable.fromIterable(integers).subscribe(System.out::println);
	...
	private void printEvenNumbersInCollectionOfNumbers() {
		List<Integer> integers = Arrays.asList(1, 2, 3, 4, 5, 6);
		Observable.fromIterable(integers).filter((integer) -> integer % 2 == 0) // only
																				// accept
																				// even
																				// numbers
	...
	private void printCollectionOfNumbersMultipliedByTen() {
		List<Integer> integers = Arrays.asList(1, 2, 3, 4, 5, 6);
		Observable.fromIterable(integers).map((integer) -> integer * 10) // convert
																			// to
																			// the
																			// number
																			// multiplied
	...
	private void printCollectionOfNumbersAsStrings() {
		List<Integer> integers = Arrays.asList(1, 2, 3, 4, 5, 6);
		Observable.fromIterable(integers).map((integer) -> "value : " + integer) // convert
																					// to
																					// String
				.subscribe(System.out::println);
	...
	private void printCollectionOfEvenNumbersMultipliedByTen() {
		...
		Observable.fromIterable(integers).filter((integer) -> integer % 2 == 0) // only
																				...
																				// even
																				// numbers
				.map((integer) -> integer * 10) // convert to the number
												// multiplied by 10
				.subscribe(System.out::println);
	...
	private void printValueFromOperation() {
		...
		Observable.fromCallable(operation).subscribe(System.out::println);
	...
	private void printResultsFromMultipleOperationsInOrder() {
		...
		Observable.concat(observableOne, observableTwo).subscribe(System.out::println);
	...
	private void printResultsFromMultipleOperationsWithoutOrdering() {
		...
		Observable.merge(observableOne, observableTwo).subscribe(System.out::println);
	...
	private void convertResultsFromMultipleOperationsToList() {
		...
		List<Integer> list = Observable.concat(observableOne, observableTwo).toList().blockingGet();
	...
	private void convertResultsFromMultipleOperationsOfSameTypeIntoOneResult() {
		...
		Observable.zip(observableOne, observableTwo, (resultOne, resultTwo) -> resultOne + resultTwo)
				.subscribe(System.out::println);
	...
	private void convertResultsFromMultipleOperationsOfDifferentTypesIntoOneResult() {
		...
		Observable
				.zip(observableOne, observableTwo, observableThree,
	...
	private void iterateOverTwoCollectionsAndPrintItemsInParallel() {
		...
		Observable.zip(alphabetsObservable, numbersObservable, (letter, number) -> letter + " = " + number)
				.subscribe(System.out::println);
