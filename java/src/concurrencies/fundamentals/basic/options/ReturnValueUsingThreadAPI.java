...
public class ReturnValueUsingThreadAPI {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ReturnValueTask task1 = new ReturnValueTask(200l);
		Thread t1 = new Thread(task1);
		t1.start();

		ReturnValueTask task2 = new ReturnValueTask(200l);
		Thread t2 = new Thread(task2);
		t2.start();

		// if task1 is still executing, then it puts the caller thread [main] to
		// wait
		// when it finishes, then notify() is called to wake up the caller
		// thread
		// same for task 2
...
class ReturnValueTask implements Runnable {
	...
	public ReturnValueTask(Long sleep) {
		this.sleep = sleep;
	}
