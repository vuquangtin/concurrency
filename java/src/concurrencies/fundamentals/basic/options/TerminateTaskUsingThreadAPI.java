...
public class TerminateTaskUsingThreadAPI {
	...
	public static void main(String[] args) {
		...
		TerminateTaskUsingThread task1 = new TerminateTaskUsingThread();
...
class TerminateTaskUsingThread implements Runnable {
	public void run() {
		for (int i = 0; i < 100000; i++) {
			...
			if (Thread.interrupted()) {
				System.out
						.println("This thread was interruped by someone calling thisThread.interrupt()");
				System.out.println("Cancelling task running in thread "
						+ Thread.currentThread().getName());
				System.out
						.println("Just illustrating that after Thread.interrupted() call, JVM reset the interrupted value to: "
								+ Thread.interrupted());
				break;
			}
		}
