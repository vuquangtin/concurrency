...
//		List<CompletableFuture<List<String>>> work1Futures = allPerson
//				.stream()
//				.map(msg -> CompletableFuture.completedFuture(msg)
//						.thenApplyAsync(s -> work1(s)))
//				.collect(Collectors.toList());

		List<CompletableFuture<String>> work2Futures = new ArrayList<>();

//		CompletableFuture<Void> allOfWork1 = CompletableFuture
//				.allOf(work1Futures.toArray(new CompletableFuture[work1Futures
//						.size()])).whenComplete(
//						(v, th) -> {
//							work1Futures.forEach(work1Result -> work2Futures
//									.addAll(work1Result
//											.getNow(null)
//											.stream()
//											.map(child -> CompletableFuture
//													.completedFuture(child))
//											.collect(Collectors.toList())));
//						});
//		allOfWork1.join();

		CompletableFuture<Void> allOfWork2 = CompletableFuture
				.allOf(work2Futures.toArray(new CompletableFuture[work2Futures
						.size()])).whenComplete((v, th) -> {
