...
public class ResubmittingScheduledThreadPoolExecutor extends
	...
	/**
	 * @param reschedule
	 *            when an exception causes a task to be aborted, reschedule it
	 *            and notify the exception listener
	 */
	...
	public ResubmittingScheduledThreadPoolExecutor(int poolSize,
			ScheduledExceptionHandler handler) {
		super(poolSize);
	...
	public ScheduledFuture<?> scheduleAtFixedRate(Runnable command,
		...
		ScheduledFuture<?> future = super.scheduleAtFixedRate(command,
		...
		if (future.isDone()) {
			try {
				...
				if (problem != null && parms != null) {
					boolean resubmitThisTask = handler
							.exceptionOccurred(problem);
					if (resubmitThisTask) {
