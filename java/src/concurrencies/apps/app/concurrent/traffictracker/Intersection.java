...
public class Intersection implements Runnable {
	...
	public void interrupt() {
		// If light is sleeping, we can call interrupt to wake it when hitting
		// "Pause" button
		thread.interrupt();
	}
	...
	public void run() {
		...
		while (isRunning.get()) {
			try {
				...
				switch (getColor()) {
				case "Green":
					j.setForeground(new Color(0, 200, 10)); // Set font color to
															// green
					j.setText(getColor());
					...
					break;
				case "Yellow":
					j.setForeground(new Color(247, 226, 35)); // Font color
																// yellow
					j.setText(getColor());
