...
public class TrafficTracker extends JFrame implements Runnable, ChangeListener {
	...
	Object[][] trafficData = { { "Car 1", car1.getPosition(), 0, 0 },
	...
	private void buildGUI() {
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createSequentialGroup().addComponent(welcome)
								.addComponent(welcome2))
				.addGap(20, 20, 20)
				.addGroup(
						layout.createParallelGroup(
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(start).addComponent(pause)
								.addComponent(stop))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE).addGap(20, 20,
								20)).addGap(20, 20, 20));
	...
	private void getData() {
		if (simIsRunning.get()) {
			...
			case "Red":
				for (Car i : carArray) {
					// If car xPosition is within 500 meters and light is red,
					// set suspend to true for car to wait
	...
	public void run() {
		while (isRunning) {
			// While running, if simulation is running, set car sliders to car
			// xPosition and get data
			if (simIsRunning.get()) {
				car1Slider.setValue(car1.getPosition());
				car2Slider.setValue(car2.getPosition());
				car3Slider.setValue(car3.getPosition());
	...
	public static void main(String[] args) {
		TrafficTracker test = new TrafficTracker();
		test.display();
		gui = new Thread(test);
