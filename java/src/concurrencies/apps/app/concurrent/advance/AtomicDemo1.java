...
public class AtomicDemo1 {
	public static void main(String[] args) {
		...
		// We will use "initialRef" object atomically across multiple threads
		// via AtomicReference class
		...
		// 2. If true then "initialRef" is replaced by "newRef". Otherwise, does
...
class Part11 {
	...
	class NumberRange {
		/**
		 * Both setLower and setUpper are check-then-act sequences, but they do
		 * not use sufficient locking to make them atomic. If the number range
		 * holds (0, 10), and one thread calls setLower(5) while another thread
		 * calls setUpper(4), with some unlucky timing both will pass the checks
		 * in the setters and both modifications will be applied. The result is
		 * that the range now holds (5, 4)an invalid state.
		 *
		 * Hence, even though we used two AtomicIntger variables but whole
		 * process is still not atomic. Which might create invalid state.
		 */
	...
	/**
	 * Fixing of the above state using a AtomicReference instead of using
	 * individual AtomicIntegers for upper and lower bounds.
	 */
