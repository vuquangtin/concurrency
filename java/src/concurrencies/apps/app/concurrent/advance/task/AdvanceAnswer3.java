...
/**
 * 3. Collect all the data from the urls in Part12 and write those data in files
 * separately. File operations must be done in separate threads.
 * 
...
public class AdvanceAnswer3 {
	...
	static class FileWritter implements Callable<String> {
		...
		FileWritter(List<String> inputArr) {
			...
			fileName = inputArr.get(1).replaceAll("^.*?//", "")
					.replaceAll("[/\\.]", "_");
		}
