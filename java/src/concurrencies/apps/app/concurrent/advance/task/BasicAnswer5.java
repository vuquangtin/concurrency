...
class BasicProducer extends Thread {
	/*-
	...
	AtomicInteger msgCount = new AtomicInteger(0);// Using atomic count for
													// better vector size
													// counting
...
class BasicConsumer extends Thread {
	BasicProducer producer;

	BasicConsumer(BasicProducer p) {
		producer = p;
	}
	...
	public static void main(String args[]) {
		...
		producer.start();
		new BasicConsumer(producer).start();
	}
}
